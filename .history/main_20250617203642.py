...
@client.tree.command(name='modal_warn', description='despiegle de modal', guild=GUILD_ID)
async def modal_warn_command(interaction: discord.Interaction, member: discord.Member):
    await interaction.response.send_modal(modal_warn.ModalWarn(cursor, conn, interaction.user.id, interaction.guild.id))
