...
async def modal_warn(interaction: discord.Interaction, cursor: sqlite3.Cursor, member: discord.Member, conn: sqlite3.Connection):
    modal = Questionnaire()
    modal.name.label = f'Razón del warn a {member.name}'
    modal.answer.label = 'Escribe la razón del warn'
    
    async def on_submit(modal_interaction: discord.Interaction):
        reason = modal.answer.value
        await WarnMiembro(modal_interaction, cursor, member, reason, conn)
