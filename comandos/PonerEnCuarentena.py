...
async def PonerEnCuarentena(interaction: discord.Interaction, cursor: sqlite3.Cursor, member: discord.Member, reason: str, conn: sqlite3.Connection):
    try:
        ...
        embed = discord.Embed(title="Usuario en cuarentena", description=f'{member.mention} ha sido puesto en cuarentena por **{reason}**', color=discord.Color.green())
        ...
        embed = discord.Embed(title="Error", description=f'No se ha podido poner en cuarentena al usuario {member.mention} por {e}', color=discord.Color.red())